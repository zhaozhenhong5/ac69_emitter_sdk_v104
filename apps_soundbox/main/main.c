...
//                    task_switch(RTC_TASK_NAME, 0,SWITCH_SPEC_TASK);
                ...
                if((os_time_get() > 200) && (!bt_status))
                {
					...
					if(get_emitter_role() == BD_ROLE_HOST) {
						...
						{
							puts("music task has created,post event\n");
							os_taskq_post_event(MUSIC_TASK_NAME,2,SYS_EVENT_DEV_ONLINE,status);
						}
						else
						{
        ...
        case SYS_EVENT_PC_IN:
			...
			if(os_time_get() > 200) {
				if(get_emitter_role() == BD_ROLE_HOST) {
					if(!g_bt_conn_status)//Ignore Dev In when bt disconnect
						break;
					set_emitter_aux_media(0);
					if(music_task_runing == 1)
					{
						puts("delete music task\n");
						os_task_delete(MUSIC_TASK_NAME);
						do{
							os_time_dly(1);
						}while(music_task_runing);
...
//            Post_msg_to_rcsp_Task(MSG_RSCP_CHANGE_WORDMODE_ACTION,0,0);
		...
		case MSG_DELETE_TASK:
			...
			else if(msg[1] == MSG_BT_DISCONN) {
				puts("delete all source media task\n");
				os_task_delete(MUSIC_TASK_NAME);
				set_emitter_aux_media(0);
			}
			break;
