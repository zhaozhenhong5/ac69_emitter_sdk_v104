...
extern void usb_audio_src_ctl(u8 toggle,u16 in_sr,u16 out_sr);
static void pc_task(void *p)
{
	...
	if(get_emitter_role() == BD_ROLE_HOST) {
		os_taskq_post(PC_TASK_NAME,1,MSG_PC_MODE_INIT);
		set_emitter_decode_media(1);
	    music_task_runing = 2;
	}
	else {
	...
	usb_audio_src_ctl(1,48000,44100);
    while(1)
    {
        memset(msg,0x00,sizeof(msg));
        os_taskq_pend(0, ARRAY_SIZE(msg), msg);
        ...
        case SYS_EVENT_DEL_TASK:
            ...
            pc_puts("PC_SYS_EVENT_DEL_TASK5\n");
				usb_audio_src_ctl(0,48000,44100);
            pc_puts("PC_SYS_EVENT_DEL_TASK6\n");
                os_task_del_res_name(OS_TASK_SELF);
            }
